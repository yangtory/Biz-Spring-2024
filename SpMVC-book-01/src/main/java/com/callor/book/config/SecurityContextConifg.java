 ...
 * Spring Security 5.4.x 이후 버전에서는 새로운 설정 코드를 사용해야 한다
 * Spring Framework 와 Spring Security 한 프로젝트에서 사용하지만
 * 		서로 연결되지 않는 Framework 
 */
@Configuration
@EnableWebSecurity
public class SecurityContextConifg extends WebSecurityConfigurerAdapter  {
	
	/*
	 * wepapp/static 폴더 : static 파일들(js,css,image 등) 을 저장하는 폴더는
	 * controller, Dispatcher 등을 통하지 않고 client 로 바로 전달하는 파일이
	 * 보관된 폴더
	 * Security 에서도 이 폴더의 파일은 관여하지 않도록 선언할 수 있는데
	 * 그 때 config(WebSecurity web) method 에서 선언한다
	 */
	@Override
...
//			.antMatchers("/**").permitAll() // 그 외 모든
			.anyRequest().permitAll() // 그 외 모든 요청은 그냥 통과
			.and().formLogin();
	}
