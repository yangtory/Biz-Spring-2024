...
public class AuthUtils {
		 ...
		 * 그 때는 다음과 같은 코드를 사용하여 객체를 생성하여 사용하면 된다
		 * PasswordEncoder encoder = new BCryptPasswordEncoder();
		 * 
		 * 하지만 Spring 프로젝트는 다수의 client 가 접근하는 Server 애플리케이션 이다.
		 * 프로젝트 전체에서는 객체가 2번만 생성하면 되지만
		 * 사용자가 접근을 할 경우 사용자 마다 2번씩 생성되는 결과가 만들어진다.
		 * 이런 상황이 되면 Spring 프로젝트의 원칙은 객체를 SingleTone 으로 만들고
		 * 필요한 곳에 DI(의존성 주입) 을 하여 사용하도록 하고 있다
		 * 
		 * 그래서 번거롭지만 Bean 으로 생성해 두는 코드를 미리 만든다
		 * Bean 생성 코드는 보통 *-context.xml 에서 만들지만
		 * @Bean Annotation 을 사용하여 Java 코드로 만들 수 있다
		 * 
