...
public class UserServiceImpl implements UserService {
	 ...
	 * UserDao 에 insert() 를 통해 회원가입 하기
	 * 회원가입을 할때 사용자의 권한을 어떻게 부여할 것인가?
	 * 
	 * 전체 회원정보 table 을 조회하여 최초의 회원가입이라면
	 * ADMIN 과 USER 권한을 부여하고
	 * 두번째 이후의 회원가입이라면
	 * USER 권한만 부여하는 시스템으로 작동
	 * 
	 * 주의
	 * @Transactional 을 사용할때 SQL 을 실행할때 발생하는 exception 을 
	 * try catch 로 절대 처리하면 안된다
	 * 만약 SQL 명령을 실행하는 과정에서 exception 이 발생하고
	 * 이를 method 에서 자체적으로 try catch 로 처리를 하면
	 * 현재 method 는 정상적으로 수행된 것으로 인식된다
	 * 따라서 transaction 처리가 무력화 되어버린다
	 * 
	 * 그래서 @Transactional 을 사용하고자 할때는
	 * 반드시 method 에서 throws 를 통해 호출한 곳으로 exception 을
	 * 보내주어야 한다. 그래야만 중간에 Transactional 이 감지하고 
	 * 처리를 하게 된다
	 */
	@Transactional
	@Override
	public int create_user(UserVO vo) throws DataAccessException {
		...
		userDao.insert(vo);
		roleDao.inserts(grantList, vo.getUsername());
		return 0;
	}
}
