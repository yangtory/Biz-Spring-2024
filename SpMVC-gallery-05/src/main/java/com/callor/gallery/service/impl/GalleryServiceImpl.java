...
public class GalleryServiceImpl implements GalleryService {
	...
	public void create_table() {
		...
		imageDao.create_table(); // tbl_gallerys 의 g_id 를 fk설정해서 순서 지키기
	}
...
//		galleryVO.setG_id(g_id);
		...
		// tbl_images 는 tbl_gallerys 와 fk 설정이되어 있어서 먼저 insert 돼야함 
	 ...
	 * 이 때 setGalleryVO() method 는 GalleryVO 객체를 Call By Reference 로
	 * 받은것과 동일하기 작동된다.
	 */
	...
	@Override
	public GalleryVO selectGalleryOne(String g_id) {
		// 대표사진 하나 찾아서
		GalleryVO vo = galleryDao.findById(g_id);
		// 나머지 사진 찾기
		List<ImagesVO> images = imageDao.findByGID(g_id);
