...
<f:form class="iolist input" modelAttribute="IO">
	<fieldset>
		<legend>매입매출 ${empty IO.io_seq ? '등록' : '수정' }</legend>
			...
			<f:input type="date" path="io_date"/></div>
			<div><label>거래시각</label>
			<f:input type="time" path="io_time"/></div>
			<div><label>상품명</label>
			...
			<div><label>단가</label><f:input path="io_price" /></div>
			<div><label>수량</label><f:input path="io_quan" /></div>
			<div><label>합계</label><f:input path="io_total"/></div>
